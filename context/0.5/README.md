# Tembo Agent Studio v0.5 — Adaptive Intelligence

> **Headline:** The agent you ship is not the agent you'll run six months later. v0.5 closes the loop *inside a single TAS deployment*: end-user corrections become targeted PRs, and divergence becomes managed variants.
>
...
After v0.4, customers have a fast authoring loop, a real operator surface, and a deep audit trail with role-based access. They are now hitting the next bottleneck:
...
The product question for v0.5 is: how do we let agents *evolve* without giving up the auditability and access discipline we just built in v0.4?

(Cross-deployment learning — sharing patterns *between* TAS instances — is a separate question and is addressed in [v0.6 (Mycelium)](../0.6/).)

## Our Solution

Three capabilities, all routed through the v0.4 governance substrate:
...
- **Correction capture surface.** Inline correction UI on every agent output that supports it; structured payload (original / correction / context / actor) attached to a v0.4 changelog event.
...
- Behavioral A/B routing inside a single agent (separate post-v0.6 conversation).
- Cross-deployment shared learning — addressed in [v0.6 (Mycelium)](../0.6/).

## Strategy

Adapt, don't drift. The platform's value compounds only if customers trust that adaptation doesn't outpace governance. v0.4 lands governance *before* v0.5 adaptive loops on purpose — the audit substrate has to be solid before we let end-user signal write back into source. We refuse to ship an "auto-apply correction" path; the PR is the contract.

## Technical Details

- **Correction model.** Append-only events tied to the run that produced the output, the user who corrected it, and (optionally) the structured form they filled. The event lands in the v0.4 changelog; RBAC applies the same way it does for any other event.
...
No. Every change is still a PR. Every PR is still subject to the v0.2 policy and the v0.4 audit + RBAC. v0.5 changes the *source* of changes, not the review surface.

### What if the coding agent produces a bad correction PR?
It gets reviewed and rejected, like any other PR. The correction event remains in the v0.4 changelog so the same correction doesn't get re-synthesized into the same bad PR.
...
That's [v0.6 (Mycelium)](../0.6/). v0.5 keeps everything inside a single TAS deployment on purpose — we want adaptation and audit to be rock-solid intra-deployment before introducing inter-deployment pattern exchange.
...
- [ ] The v0.4 changelog cleanly absorbs correction and variant events without a separate audit surface.

## Open Questions Before v0.6
...
- What's the right pattern abstraction for v0.6 to export — diffs? Variant scopes? Correction classifications?
