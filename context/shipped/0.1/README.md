...
- ~~The mockup treats `harness` and `model` as first-class agent fields ([US-0.1-07](./USER_STORIES.md#us-01-07--framework-and-model-as-first-class-agent-fields)). Should the v0.1 starter templates declare a default harness/model pair, or should every workspace pick its own at onboarding?~~ **Resolved (and reframed):** the badge surfaces **framework** (Pydantic AgentSpec; Cargo AI when import lands) + **model** instead of harness. Starter ships `model: anthropic:claude-sonnet-4-6`; framework is implied by the format. Harness as a separate concept is deferred to v0.3+ alongside raw-coding-agent flows. See [`AGENT_FORMAT.md`](./AGENT_FORMAT.md#framework--model-as-first-class-fields-us-01-07).
