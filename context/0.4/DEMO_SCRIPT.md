...
**Target audience:** prospective customer with a live agent in production at a v0.3 baseline — typically a product owner and an EM.
**Target duration:** 25–35 minutes.
**Goal of the demo:** prove that adaptation can be both fast and governed *inside a single TAS deployment*. Cross-deployment learning (Mycelium) has its own demo in [v0.5](../0.5/DEMO_SCRIPT.md).
...
### 7. Wrap and pointer to v0.5 (33:00 – 35:00)
**Show:** the five-phase roadmap with 0.1–0.4 marked complete and 0.5 (Mycelium) queued.
**Say:**
> "v0.1 made TAS deployable, v0.2 made it iterable, v0.3 made it auditable, v0.4 made it adaptive — all inside one deployment. v0.5 (Mycelium) is the optional next step: letting two TAS deployments learn from each other under explicit policy. That's a separate conversation with a separate demo, because the trust model is different."
...
- **"Can our agents learn from another company's TAS deployment?"** Only under v0.5 (Mycelium), which is opt-in and off by default. v0.4 stays inside your deployment on purpose.
- **"What's after v0.4?"** v0.5 (Mycelium), then depth in specific verticals and platform extensibility.
