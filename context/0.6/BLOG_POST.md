# Tembo Agent Studio v0.6: Mycelium — Shared Learning, On Your Terms
...
TAS v0.6 introduces **Tembo Mycelium**, the optional substrate for that exchange. Everything we built through v0.5 — corrections-as-PRs, variants, the v0.4 audit timeline, RBAC — was the closed loop *inside* a single deployment. Mycelium extends that loop *between* deployments. Optionally. Under explicit policy. Without ever turning trust into a default.

## Why Mycelium is its Own Phase

We considered shipping Mycelium inside v0.5. We decided not to, on purpose.

Mycelium is a fundamentally different trust conversation. v0.1–v0.5 ask: *can you trust this system to manage your agents inside your walls?* Mycelium asks: *under what conditions, if any, do you want this system to exchange behavior with another organization's deployment?*
...
This is the boundary that makes Mycelium *not* a data-sharing feature. Patterns are about *what worked*, not *what was said*. The schema is versioned, designed with backward-compatible reads, and reviewed by regulated-customer compliance teams before v0.6 GA.

## Audit, Inherited

Mycelium imports land in the same v0.4 changelog as every other change. There is no separate "AI changes" surface or "Mycelium activity" tab. An auditor asking "where did this behavior come from?" gets the same answer for an internal correction, an engineer's PR, and a Mycelium-sourced import: the changelog, with attribution.
...
- Not federated agent execution. v0.6 exchanges patterns between deployments; it does not execute agents across them.
- Not on by default. Default is, and will remain, island.
- Not a bypass for v0.5 review surfaces. Imports produce PRs.
...
- v0.3 earns trust to **be operated** day to day.
- v0.4 earns trust to **explain every change** under explicit access policy.
- v0.5 earns trust to **adapt from end-user signal**.
- v0.6 earns trust to **learn beyond your own walls** — and only because the five steps below it are solid.

A customer who reaches v0.6 has already seen TAS adapt under their own audit trail for months. The question Mycelium asks them is qualitatively different at that point.

## What's Next

Beyond v0.6, the roadmap shifts to depth in specific verticals (regulated industries, multilingual operations) and to platform extensibility. Those will get their own phase documents when planned.

If v0.5 proved TAS can grow, v0.6 proves TAS can grow *together with peer deployments* — never by default, never without attribution, always under your audit trail.
