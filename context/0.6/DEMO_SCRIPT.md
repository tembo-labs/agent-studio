...
**Target audience:** enterprise architect, compliance lead, and platform owner at a customer already operating TAS at v0.5. Mycelium is a *different conversation* than v0.1–v0.5; do not stack this demo with the adaptive-intelligence one.
...
- Two TAS deployments at v0.5 baseline (call them `acme` and `globex`) with a pre-negotiated bilateral peer relationship.
...
**Say:**
> "Everything you've seen through v0.5 keeps your agents' evolution inside your walls. v0.6 — Mycelium — is the optional substrate for two TAS deployments to learn from each other. Three things to know up front. One: this is off by default. Two: it never moves data, only anonymized behavioral patterns. Three: imports land as PRs, on your audit trail, under the same RBAC, just like everything else. If by the end of this demo you decide you want to stay in island mode, that is a supported outcome, not a missed sale."
...
- Show the v0.4 changelog entry on `globex` with originating attribution + the PR link.
- Walk through the PR review like any other v0.5 correction PR.
...
### 8. Wrap (27:00 – 30:00)
**Show:** the six-phase roadmap, all complete.
**Say:**
> "v0.1 through v0.5 made TAS deployable, iterable, operable, auditable, and adaptive — inside your walls. v0.6 is the optional next floor: learning across deployments under explicit policy. The trust ladder runs in this direction on purpose. If you want to stay on the v0.5 floor forever, that's a supported choice. If you want to take the v0.6 step with a specific peer, we'll set up the bilateral relationship together."
...
- **"Does v0.6 require v0.5?"** Yes. Mycelium builds on v0.5's correction, variant, and (via v0.4) changelog substrate.
- **"What's after v0.6?"** Depth in specific verticals and platform extensibility. Phase docs when planned, not before.
