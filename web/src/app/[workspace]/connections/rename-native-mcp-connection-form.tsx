...
import {
  ...
  type RenameNativeMcpConnectionFormState,
} from "./native-mcp-actions";

const INITIAL: RenameNativeMcpConnectionFormState = {};

// Inline rename for a native-MCP connection slot — mirror of
// RenameComposioConnectionForm so the two surfaces feel identical.
// Toggles between a "Rename" link (idle) and a small input + Save /
// Cancel (editing); surfaces the side-effect warning so users know
// any agent file pinning the old name needs an update.

export function RenameNativeMcpConnectionForm({
  workspaceSlug,
  connectionId,
  currentName,
}: {
