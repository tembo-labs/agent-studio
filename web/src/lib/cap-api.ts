...
export function buildCreateAgentPrompt(args: {
  ...
  return [
    ...
    "needs Gmail, declare `- gmail` and TAS will surface a Connect button",
    "for it. Don't scaffold provider SDK config, environment variables,",
    "or credential plumbing in the agent file — the runtime injects the",
    ...
    "(set on the automation form). When a user has multiple accounts of",
    "the same toolkit (e.g. work + personal Gmail), use the named slot",
    "form `- gmail: { name: work }`. Without an explicit name the agent",
    "uses the user's `default` slot for that toolkit.",
    "",
